...
//        $missedDataAlerts = $crawler->filter('.alert.alert-warning');
...
//        $this->assertEquals(intval($hasForm), $missedDataAlerts->count());
        ...
        if ($hasForm) {
            $this->assertEquals(0, $confirmButton->count());
        ...
        if ($serviceName === 'ostrovok') {
            $roomsCrawler = $this->client->followRedirect();
            $this->assertEquals(1, $roomsCrawler->filter('form[name="mbh_bundle_channelmanagerbundle_rooms_type"]')->count());
        } else {
    ...
    public function testIndexAction(string $serviceName)
    {
        ...
        $channelManagersWithAllDataFilledInIntro = ['ostrovok'];
        if (in_array($serviceName, $channelManagersWithAllDataFilledInIntro)) {
        ...
        $this->assertEquals('http://localhost/management/channelmanager/' . $serviceName . '/' . 'room', $roomsCrawler->getUri());
    }
    ...
    public function testRoomAction(string $serviceName)
    {
        ...
        $roomsForm = $roomsFormCrawler->form([
            $this->getRoomFormName($roomsFormName, ChannelManagerServiceMock::FIRST_ROOM_ID, $serviceName) => $roomTypes[0]->getId(),
            $this->getRoomFormName($roomsFormName, ChannelManagerServiceMock::SECOND_ROOM_ID, $serviceName) => $roomTypes[1]->getId()
        ]);
        $this->client->submit($roomsForm);

        $this->assertEquals(302, $this->client->getResponse()->getStatusCode());
        $tariffsCrawler = $this->client->followRedirect();
        $this->assertEquals('http://localhost/management/channelmanager/' . $serviceName . '/' . 'tariff', $tariffsCrawler->getUri());
    }
