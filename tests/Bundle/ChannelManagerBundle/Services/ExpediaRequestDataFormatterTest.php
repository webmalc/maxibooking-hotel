...
class ExpediaRequestDataFormatterTest extends UnitTestCase
{
    ...
    {
        $password = $this->container->getParameter('expedia_pass');

        return '<?xml version="1.0"?>
<AvailRateUpdateRQ xmlns="http://www.expediaconnect.com/EQC/AR/2011/06"><Authentication username="EQCMaxibooking" password="'
            . $password . '"/><Hotel id="' . self::EXPEDIA_HOTEL_ID . '"/>'
...
';
    ...
    private function getServiceRoomIds()
    {
        return array_map(function (int $number) {
            return 'room' . $number;
        }, range(1, $this->getHotel()->getRoomTypes()->count()));
    }
}
