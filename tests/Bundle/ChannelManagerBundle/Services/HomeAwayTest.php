...
class HomeAwayTest extends UnitTestCase
{
    ...
    public function testSecondPullOrders()
    {
        ...
        $orders = $this->dm
            ->getRepository(Order::class)
            ->findBy(['channelManagerType' => HomeAway::NAME]);
    ...
    public function testGenerateRoomCalendar()
    {
        ...
        $generatedRoomCalendar = $this->container->get('mbh.home_away')->generateRoomCalendar($roomType);
        $calendar = new ICal($generatedRoomCalendar);
        $events = $calendar->events();

        $lastEvent = end($events);
        $dayAfterLastRoomCache = new \DateTime(RoomCacheData::PERIOD_LENGTH_STR);
        $lastDayOfSentData = new \DateTime('midnight + 1 day +' . HomeAway::PERIOD_LENGTH);
...
END:VEVENT\n";
    ...
    private function initHomeAwayConfig()
    {
        $config = $this->dm->getRepository(HomeAwayConfig::class)->findOneBy([]);
        if ($config === null) {
            /** @var Hotel $hotel */
            $hotel = $this->dm->getRepository('MBHHotelBundle:Hotel')->findOneBy(['isDefault' => true]);
            $roomType = $this->dm
            ...
            $config = (new HomeAwayConfig())
                ...
                ->addRoom((new HomeAwayRoom())->setSyncUrl('/some_fiction_url')->setRoomType($roomType));
            $hotel->setHomeAwayConfig($config);
    ...
    {
        $airbnbEndDate = $url === '/some_fiction_url' ? '20180401' : '20210401';
