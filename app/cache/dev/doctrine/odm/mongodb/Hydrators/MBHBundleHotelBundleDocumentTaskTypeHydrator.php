 ...
 */
class MBHBundleHotelBundleDocumentTaskTypeHydrator implements HydratorInterface
{
    private $dm;
    private $unitOfWork;
    private $class;
    ...
    public function hydrate($document, $data, array $hints = array())
    {
        ...
        /** @ReferenceOne */
        if (isset($data['defaultUserGroup'])) {
            $reference = $data['defaultUserGroup'];
            if (isset($this->class->fieldMappings['defaultUserGroup']['simple']) && $this->class->fieldMappings['defaultUserGroup']['simple']) {
                $className = $this->class->fieldMappings['defaultUserGroup']['targetDocument'];
                $mongoId = $reference;
            } else {
                $className = $this->unitOfWork->getClassNameForAssociation($this->class->fieldMappings['defaultUserGroup'], $reference);
                $mongoId = $reference['$id'];
            }
            $targetMetadata = $this->dm->getClassMetadata($className);
            $id = $targetMetadata->getPHPIdentifierValue($mongoId);
            $return = $this->dm->getReference($className, $id);
            $this->class->reflFields['defaultUserGroup']->setValue($document, $return);
            $hydratedData['defaultUserGroup'] = $return;
        }

        /** @ReferenceOne */
        if (isset($data['roomStatus'])) {
            $reference = $data['roomStatus'];
            if (isset($this->class->fieldMappings['roomStatus']['simple']) && $this->class->fieldMappings['roomStatus']['simple']) {
                $className = $this->class->fieldMappings['roomStatus']['targetDocument'];
                $mongoId = $reference;
            } else {
                $className = $this->unitOfWork->getClassNameForAssociation($this->class->fieldMappings['roomStatus'], $reference);
                $mongoId = $reference['$id'];
            }
            $targetMetadata = $this->dm->getClassMetadata($className);
            $id = $targetMetadata->getPHPIdentifierValue($mongoId);
            $return = $this->dm->getReference($className, $id);
            $this->class->reflFields['roomStatus']->setValue($document, $return);
            $hydratedData['roomStatus'] = $return;
        }
